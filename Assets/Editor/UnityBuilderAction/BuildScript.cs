...
namespace UnityBuilderAction
{
    public static class BuildScript
    {
        ...
        private static void WinBuild()
        {
            var dataPath = UnityEngine.Application.dataPath;
            exit = false;
            Build(BuildTarget.StandaloneWindows64, 0, dataPath + "\\Builds\\BuildWin64\\Game.exe");
        }
        ...
        private static void BuildAndroid()
        {
            var dataPath = UnityEngine.Application.dataPath;
            exit = false;
            Build(BuildTarget.Android, 0, dataPath + "\\Builds\\Android\\Game.apk");
        }
        ...
        private static void ExitWithResult(BuildResult result)
        {
            switch (result)
            {
                ...
                case BuildResult.Failed:
                    ...
                    if (exit) EditorApplication.Exit(101);
                    break;
                case BuildResult.Cancelled:
                    Console.WriteLine("Build cancelled!");
                    if (exit) EditorApplication.Exit(102);
                    break;
                case BuildResult.Unknown:
                default:
                    Console.WriteLine("Build result is unknown!");
                    if (exit) EditorApplication.Exit(103);
                    break;
            }
        }
    }
